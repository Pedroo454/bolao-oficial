...
export const AdminPanel: React.FC<AdminPanelProps> = ({ onLogout }) => {
  const [activeTab, setActiveTab] = useState<"users" | "matches" | "rodadas" | "bracket" | "testes">("users");
  ...
  // States for manual opening/closing of betting rounds
  const [roundStatuses, setRoundStatuses] = useState<Record<number, "open" | "closed">>({
    1: "open",
    2: "closed",
    3: "closed",
    4: "closed",
  ...
  const handleToggleRoundStatus = async (rNum: number, newStatus: "open" | "closed") => {
    sounds.playClick();
    try {
      await setDoc(doc(db, "rounds", String(rNum)), { status: newStatus });
      showFeedback(`Rodada ${rNum} foi ${newStatus === "open" ? "ABERTA" : "FECHADA/TRAVADA"} com sucesso para palpites!`, "success");
    } catch (err) {
      handleFirestoreError(err, OperationType.WRITE, `rounds/${rNum}`);
    }
  };

  // Local Simulator states
  const [simHomeReal, setSimHomeReal] = useState<string>("2");
  const [simAwayReal, setSimAwayReal] = useState<string>("1");
  const [simHomeBet, setSimHomeBet] = useState<string>("2");
  const [simAwayBet, setSimAwayBet] = useState<string>("1");
  
  // Selected user for audit analysis
  const [testSelectedUserId, setTestSelectedUserId] = useState<string>("all_mock");

  const calcSimulatedPoints = (hr: number, ar: number, hb: number, ab: number) => {
    // Exact match = 3 points
    if (hr === hb && ar === ab) {
      return { POINTS: 3, DESC: "🎯 CRAVOU O PLACAR EXATO! O aluno acertou exatamente o número de gols de ambos os lados. Ganha os 3 pontos máximos." };
    }
    
    // Correct outcome (Sign match) = 1 point
    const realDiff = hr - ar;
    const betDiff = hb - ab;
    const realSign = Math.sign(realDiff);
    const betSign = Math.sign(betDiff);
    
    if (realSign === betSign) {
      return { POINTS: 1, DESC: "👍 ACERTOU O VENDEDOR/EMPATE! O aluno acertou quem venceria ou se seria empate, mas errou o placar exato. Ganha 1 ponto." };
    }
    
    // Incorrect outcome = 0 points
    return { POINTS: 0, DESC: "❌ ERROU O VENDEDOR E PLACAR! O aluno apostou em um resultado completamente diferente do oficial. Ganha 0 pontos." };
  };

  // Listen to Users, Matches, Bets, and Rounds databases in real-time
  useEffect(() => {
    ...
    const unsubRounds = onSnapshot(collection(db, "rounds"), (snapshot) => {
      const statuses: Record<number, "open" | "closed"> = {
        1: "open",
        2: "closed",
        3: "closed",
        4: "closed",
      ...
      snapshot.forEach((doc) => {
        const rNum = parseInt(doc.id);
        if (!isNaN(rNum)) {
          statuses[rNum] = doc.data().status === "open" ? "open" : "closed";
        }
      });
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 md:px-0 py-6" id="dashboard_admin">
      ...
      <div className="flex bg-zinc-900 border border-zinc-800 p-1.5 rounded-xl mb-6 overflow-x-auto select-none scrollbar-thin scrollbar-thumb-zinc-800 scrollbar-track-transparent gap-1">
        <button
          onClick={() => { sounds.playClick(); setActiveTab("users"); }}
          className={`flex-1 flex items-center justify-center gap-2 py-3 px-4 rounded-lg font-sans text-xs md:text-sm font-bold transition-all cursor-pointer flex-shrink-0 ${
            activeTab === "users"
        ...
        <button
          onClick={() => { sounds.playClick(); setActiveTab("testes"); }}
          className={`flex-grow md:flex-1 flex items-center justify-center gap-2 py-3 px-4 rounded-lg font-sans text-xs md:text-sm font-bold transition-all cursor-pointer flex-shrink-0 ${
            activeTab === "testes"
              ? "bg-orange-500 text-white shadow-md shadow-orange-950/50"
              : "text-zinc-400 hover:text-white"
          }`}
      ...
      {activeTab === "rodadas" && (
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 shadow-xl space-y-6">
          ...
          {/* CONTROLES DE ABERTURA / FECHAMENTO MANUAL */}
          <div className="bg-zinc-950/40 p-5 rounded-3xl border border-zinc-850/80 space-y-4">
            <div className="flex items-center gap-2 border-b border-zinc-850 pb-2.5">
              ...
              <h3 className="text-sm font-black text-zinc-100 uppercase tracking-widest font-mono">
                Controle Manual de Abertura e Fechamento de Rodadas
              </h3>
            </div>
            
            <p className="text-xs text-zinc-400 font-sans leading-relaxed">
              Aqui você pode liberar ou travar manualmente os palpites para cada rodada do bolão da sala 3A Américo. Quando uma rodada está fechada, os alunos não conseguem enviar ou editar seus palpites.
            </p>

            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 pt-2">
              {[1, 2, 3, 4].map((rNum) => {
                const rName = rNum === 1 ? "Grupos A-D" 
                             : rNum === 2 ? "Grupos E-H" 
                             : rNum === 3 ? "Grupos I-L" 
                             : "Mata-Mata Completo";
                const currentStatus = roundStatuses[rNum] || "closed";
                const isOpen = currentStatus === "open";

                return (
                  <div 
                    key={rNum} 
                    className={`p-4 rounded-2xl border flex flex-col justify-between gap-3 font-sans transition-all shadow ${
                      isOpen 
                        ? "bg-zinc-900 border-emerald-500/20 shadow-emerald-950/10" 
                        : "bg-zinc-900 border-zinc-800 shadow-rose-950/5"
                    }`}
                  >
                    <div>
                      <div className="flex items-center justify-between gap-1.5">
                        ...
                        <span className={`px-2 py-0.5 rounded text-[9px] font-mono font-bold tracking-tight ${
                          isOpen 
                            ...
                            : "bg-rose-950/50 text-rose-450 border border-rose-500/10"
                        }`}>
                          {isOpen ? "🔓 ABERTA" : "🔒 BLOQUEADA"}
                        </span>
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 mt-2 pt-2 border-t border-zinc-850">
                      <button
                        onClick={() => handleToggleRoundStatus(rNum, "open")}
                        className={`py-1.5 px-2 rounded-lg text-[10px] font-black tracking-wide uppercase transition-all flex items-center justify-center gap-1 cursor-pointer ${
                          isOpen 
                            ? "bg-zinc-950 text-zinc-600 border border-zinc-850 cursor-not-allowed opacity-50" 
                      ...
                      <button
                        onClick={() => handleToggleRoundStatus(rNum, "closed")}
                        className={`py-1.5 px-2 rounded-lg text-[10px] font-black tracking-wide uppercase transition-all flex items-center justify-center gap-1 cursor-pointer ${
                          !isOpen 
                            ? "bg-zinc-950 text-zinc-600 border border-zinc-850 cursor-not-allowed opacity-50" 
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-zinc-850 pb-4 gap-4 pt-4">
            <h2 className="text-xl font-bold font-sans text-white flex items-center gap-2">
      ...
      {activeTab === "testes" && (
        <div className="space-y-6 animate-fade-in">
          ...
          <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-3xl shadow-xl relative overflow-hidden">
            <div className="absolute right-0 top-0 w-44 h-44 bg-orange-500/5 rounded-full blur-3xl -z-1" />
            <h2 className="text-xl font-black text-white flex items-center gap-2">
              <Sparkles className="text-orange-500 animate-pulse" size={24} />
              Área de Testes e Simulador de Pontuação
            </h2>
            <p className="text-xs text-zinc-400 mt-1 max-w-xl">
              Use esta funcionalidade para simular resultados e conferir detalhadamente como a pontuação está sendo contabilizada e ver as apostas de cada aluno ao vivo!
            </p>
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pb-8">
            
            {/* COLUMN 1: SIMULADOR DE PLACAR INDIVIDUAL */}
            <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-3xl shadow-xl space-y-4">
              <h3 className="text-sm font-black text-zinc-200 uppercase tracking-wider flex items-center gap-2 border-b border-zinc-850 pb-3">
                <span className="w-1.5 h-3 bg-orange-500 rounded-sm" />
                1. Simulador de Bolão Rápido
              </h3>
              
              <p className="text-xs text-zinc-500 font-sans">
                Insira valores hipotéticos abaixo para simular instantaneamente o cálculo da pontuação oficial.
              </p>

              <div className="grid grid-cols-2 gap-4 pt-2">
                
                {/* Placar Oficial */}
                <div className="bg-zinc-950 p-4 rounded-2xl border border-zinc-850/80 space-y-3">
                   <div className="text-[10px] font-mono uppercase font-extrabold text-zinc-500 border-b border-zinc-900 w-fit pb-1">
                     ⚽ Placar Real Oficial
                   </div>
                   <div className="flex items-center justify-around gap-2 pt-1">
                     <div className="flex flex-col items-center gap-1">
                       <span className="text-[10px] font-mono text-zinc-500 font-bold">Mandante</span>
                       <input 
                         type="number"
                         min="0"
                         max="99"
                         value={simHomeReal}
                         onChange={(e) => setSimHomeReal(e.target.value)}
                         className="w-12 h-12 bg-zinc-900 border border-zinc-800 text-center font-mono font-black text-white rounded-lg outline-none focus:border-orange-500 text-lg"
                       />
                     </div>
                     <span className="text-zinc-650 font-mono text-sm self-end pb-3">x</span>
                     <div className="flex flex-col items-center gap-1">
                       <span className="text-[10px] font-mono text-zinc-500 font-bold">Visitante</span>
                       <input 
                         type="number"
                         min="0"
                         max="99"
                         value={simAwayReal}
                         onChange={(e) => setSimAwayReal(e.target.value)}
                         className="w-12 h-12 bg-zinc-900 border border-zinc-800 text-center font-mono font-black text-white rounded-lg outline-none focus:border-orange-500 text-lg"
                       />
                     </div>
                   </div>
                ...
                {/* Palpite do Aluno */}
                <div className="bg-zinc-955 p-4 rounded-2xl border border-zinc-855/80 space-y-3">
                   <div className="text-[10px] font-mono uppercase font-extrabold text-zinc-500 border-b border-zinc-900 w-fit pb-1">
                     🎯 Palpite do Aluno
                   </div>
                   <div className="flex items-center justify-around gap-2 pt-1">
                     <div className="flex flex-col items-center gap-1">
                       ...
                       <input 
                         ...
                         max="99"
                         value={simHomeBet}
                         onChange={(e) => setSimHomeBet(e.target.value)}
                         className="w-12 h-12 bg-zinc-900 border border-zinc-805 text-center font-mono font-black text-emerald-400 rounded-lg outline-none focus:border-emerald-500 text-lg"
                       />
                     </div>
                     <span className="text-zinc-650 font-mono text-sm self-end pb-3">x</span>
                     <div className="flex flex-col items-center gap-1">
                       ...
                       <input 
                         ...
                         max="99"
                         value={simAwayBet}
                         onChange={(e) => setSimAwayBet(e.target.value)}
                         className="w-12 h-12 bg-zinc-900 border border-zinc-805 text-center font-mono font-black text-emerald-400 rounded-lg outline-none focus:border-emerald-500 text-lg"
                       />
                     </div>
                   </div>
              ...
              {(() => {
                const hr = parseInt(simHomeReal);
                const ar = parseInt(simAwayReal);
                const hb = parseInt(simHomeBet);
                const ab = parseInt(simAwayBet);

                if (isNaN(hr) || isNaN(ar) || isNaN(hb) || isNaN(ab)) {
                  return (
                    <div className="p-4 bg-zinc-950 border border-dashed border-zinc-850 text-center rounded-2xl text-xs text-zinc-500 font-sans">
                      Preencha todos os campos para executar a simulação da regra de pontos.
                    </div>
                  );
                }

                const result = calcSimulatedPoints(hr, ar, hb, ab);
                
                return (
                  <div className="bg-zinc-950 border border-zinc-850 rounded-2xl p-5 space-y-3.5">
                    <div className="flex items-center justify-between border-b border-zinc-900 pb-2">
                      <span className="text-xs font-mono font-extrabold text-zinc-400 uppercase">Resultado do Cálculo</span>
                      <span className={`px-3 py-1 text-xs font-black rounded-lg ${
                        ...
                        +{result.POINTS} {result.POINTS === 1 ? "PONTO" : "PONTOS"}
                      </span>
                    </div>

                    <div className="space-y-1.5">
                      <p className="text-xs font-sans text-orange-400 font-extrabold flex items-center gap-1">✓ Critério Oficial Aplicado:</p>
                      <p className="text-xs text-zinc-300 leading-relaxed font-sans">{result.DESC}</p>
                    </div>

                    <div className="bg-zinc-900/60 rounded-xl p-3 border border-zinc-900 py-3 text-[10px] font-mono text-zinc-500 space-y-1 leading-normal">
                      <div>• Placar Final: <strong className="text-white">{hr} x {ar}</strong> ({hr > ar ? "Vitória Mandante" : hr < ar ? "Vitória Visitante" : "Empate"})</div>
                      <div>• Palpitado: <strong className="text-emerald-400">{hb} x {ab}</strong> ({hb > ab ? "Vitória Mandante" : hb < ab ? "Vitória Visitante" : "Empate"})</div>
                    </div>
                  </div>
                );
              })()}
            ...
            {/* COLUMN 2: ANALISADOR DE APOSTAS DO ALUNO */}
            <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-3xl shadow-xl space-y-4">
              <h3 className="text-sm font-black text-zinc-200 uppercase tracking-wider flex items-center gap-2 border-b border-zinc-850 pb-3">
                <span className="w-1.5 h-3 bg-orange-500 rounded-sm" />
                2. Consultar Palpites de Alunos ao Vivo
              </h3>

              <div className="space-y-1.5">
                <label className="block text-[10px] font-mono text-zinc-500 uppercase font-black">Selecione um Aluno da Sala 3A Américo:</label>
                <select
                  value={testSelectedUserId}
                  onChange={(e) => { sounds.playClick(); setTestSelectedUserId(e.target.value); }}
                  className="w-full bg-zinc-950 border border-zinc-850 px-3 py-2.5 rounded-xl text-xs font-sans font-bold text-white outline-none focus:border-orange-500"
                >
                  <option value="all_mock">-- Escolha um Aluno da Lista --</option>
                  {users.map((u) => (
                    <option key={u.id} value={u.id}>
                      {u.name} (Pontuação Atual no Ranking: {u.score} pts)
                    </option>
                  ))}
                </select>
              </div>

              {testSelectedUserId !== "all_mock" ? (() => {
                const user = users.find(u => u.id === testSelectedUserId);
                if (!user) return null;

                // Grab user's registered bets
                const userBets = bets.filter(b => b.userId === user.id);

                return (
                  <div className="space-y-3 pt-1">
                    <div className="flex items-center justify-between text-[11px] font-mono text-zinc-500 border-b border-zinc-950 pb-2">
                      <span>Palpites enviados: <strong className="text-white">{userBets.length}</strong></span>
                      <span className="text-zinc-400">Pontuação Total: <strong className="text-white">{user.score} pts</strong></span>
                    </div>

                    {userBets.length === 0 ? (
                      <div className="text-center py-10 text-xs font-mono text-zinc-650 bg-zinc-950 rounded-2xl border border-dashed border-zinc-850">
                        Nenhum palpite enviado por este aluno ainda no banco de dados.
                      </div>
                    ) : (
                      <div className="space-y-2 max-h-[300px] overflow-y-auto pr-1">
                        {userBets.map((b) => {
                          ...
                          return (
                            <div key={b.id} className="bg-zinc-950 border border-zinc-855 rounded-xl p-3 flex flex-col gap-2">
                              {/* Row header */}
                              <div className="flex justify-between items-center text-[9px] font-mono text-zinc-500 uppercase">
                                <span className="text-zinc-400 font-extrabold">{m.group}</span>
                                <span>{m.homeTeam} x {m.awayTeam}</span>
                              </div>
                              ...
                              <div className="flex items-center justify-between">
                                ...
                                <div className="flex items-center gap-1.5 text-xs">
                                  <span className="text-[10px] text-zinc-500">Palpite:</span>
                                  <span className="font-mono text-emerald-400 font-bold bg-emerald-950/20 px-2.5 py-1 rounded border border-emerald-500/15">
                                    {b.homeTeamBet} x {b.awayTeamBet}
                                  </span>
                                </div>

                                {/* Official Representation */}
                                <div className="flex items-center gap-1.5 text-xs">
                                  <span className="text-[10px] text-zinc-500">Placar:</span>
                                  <span className="font-mono text-white font-bold bg-zinc-900 px-2.5 py-1 rounded border border-zinc-800">
                                    {m.status === "finished" ? `${m.homeScore} x ${m.awayScore}` : "Pendente"}
                                  </span>
                                </div>

                                {/* Calculated Points dynamically */}
                                {m.status === "finished" ? (() => {
                                  const pts = calcSimulatedPoints(m.homeScore!, m.awayScore!, b.homeTeamBet, b.awayTeamBet).POINTS;
                                  return (
                                  ...
                                  <span className="text-[10px] font-sans text-orange-400 font-semibold uppercase animate-pulse">Aguardando...</span>
                                )}
                              </div>
                            </div>
                          );
                ...
                <div className="text-center py-16 text-xs text-zinc-500 font-sans border border-dashed border-zinc-850 rounded-2xl">
                  Selecione um aluno acima para carregar sua ficha de acompanhamento detalhada de palpites e pontos.
                </div>
              )}
