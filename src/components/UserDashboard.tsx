...
export const UserDashboard: React.FC<UserDashboardProps> = ({ currentUser, onLogout }) => {
  ...
  const [selectedBetGroup, setSelectedBetGroup] = useState<string>("Grupo A");
  ...
  const handleSelectBetGroup = (groupName: string) => {
    sounds.playClick();
    setSelectedBetGroup(groupName);
    if (["Grupo A", "Grupo B", "Grupo C", "Grupo D"].includes(groupName)) {
      setActiveBetRound(1);
    } else if (["Grupo E", "Grupo F", "Grupo G", "Grupo H"].includes(groupName)) {
      setActiveBetRound(2);
    } else if (["Grupo I", "Grupo J", "Grupo K", "Grupo L"].includes(groupName)) {
      setActiveBetRound(3);
    } else {
      setActiveBetRound(4); // Mata-Mata
    }
  };

  const isMatchBetLocked = (match: MatchData) => {
    // 1. Check if the round is manual open/closed
    const roundStatus = roundStatuses[match.roundNumber] || "closed";
    if (roundStatus === "closed") {
      return true;
    }
    // 2. Finished match is locked
    if (match.status === "finished") {
      return true;
    }
    // 3. Kickoff lock: 5 minutes before match starts
    const gameStart = new Date(match.date).getTime();
    const lockTimestamp = gameStart - 5 * 60 * 1000;
  ...
  useEffect(() => {
    ...
    // 4. Fetch rounds manual opening/closing statuses
  ...
  useEffect(() => {
    const calculateCountdown = () => {
      ...
      const isManualClosed = (roundStatuses[activeBetRound] || "closed") === "closed";

      if (isManualClosed) {
        setTimeLeftString("MANUALMENTE FECHADO PELO ADMIN 🔒");
      ...
      if (diffMs <= 0) {
        setTimeLeftString("FECHADO PARA APOSTAS (Tempo Esgotado) 🔒");
        setIsRoundLocked(true);
      } else {
        setIsRoundLocked(false);
  ...
  }, [matches, activeBetRound, roundStatuses]);
  ...
  const handleSaveIndividualBet = async (matchId: string) => {
    ...
    if (isMatchBetLocked(matchData)) {
      showNotification("Esta aposta está travada ou pertence a uma rodada futura ainda fechada!", "error");
      return;
    }
    ...
    <div className="w-full max-w-7xl mx-auto px-4 md:px-0 py-4" id="student_dashboard">
      ...
      {activeSubTab === "bets" && (
        <div className="space-y-6">
          ...
          {/* Group selector menu perfectly identical to World Cupstandings menu */}
          <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-6 shadow-xl space-y-4">
            <div>
              <label className="block text-[10px] font-mono font-bold text-zinc-500 uppercase tracking-widest mb-1.5">
                Escolher Grupo ou Fase para Apostar
              </label>
              <div className="flex gap-1.5 overflow-x-auto pb-3 scrollbar-thin scrollbar-thumb-zinc-800 scrollbar-track-transparent select-none">
                {["Grupo A", "Grupo B", "Grupo C", "Grupo D", "Grupo E", "Grupo F", "Grupo G", "Grupo H", "Grupo I", "Grupo J", "Grupo K", "Grupo L", "Mata-Mata"].map((groupName) => {
                  const label = groupName === "Mata-Mata" ? "Mata-Mata" : groupName.replace("Grupo ", "Grupo ");
                  const isSelected = selectedBetGroup === groupName;
                  return (
                    <button
                      key={groupName}
                      onClick={() => handleSelectBetGroup(groupName)}
            ...
            {/* Countdown / Informative Banner */}
            {(() => {
              let selectBetGroupRound = 1;
              if (["Grupo E", "Grupo F", "Grupo G", "Grupo H"].includes(selectedBetGroup)) {
                selectBetGroupRound = 2;
              } else if (["Grupo I", "Grupo J", "Grupo K", "Grupo L"].includes(selectedBetGroup)) {
                selectBetGroupRound = 3;
              } else if (selectedBetGroup === "Mata-Mata") {
                selectBetGroupRound = 4;
              }

              const isGroupOpenForBetting = roundStatuses[selectBetGroupRound] === "open";
              
              if (!isGroupOpenForBetting) {
                return (
                  <div className="flex flex-col md:flex-row items-center gap-4 p-4 bg-amber-950/20 border border-amber-500/30 rounded-xl">
                    <Lock size={20} className="text-amber-500 flex-shrink-0" />
                    <div>
                      <div className="text-sm font-bold text-amber-400">RODADA BLOQUEADA PELO ADMINISTRADOR 🔒</div>
                      <div className="text-xs text-zinc-400 mt-0.5 font-sans">
                        Esta rodada/fase de apostas encontra-se fechada temporariamente por decisão da administração da sala.
                      </div>
                    </div>
                  </div>
                );
              ...
              return (
                <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 p-4 bg-zinc-950/80 border border-zinc-850 rounded-xl">
                  <div>
                    <div className="text-[10px] uppercase font-mono font-black tracking-wider text-zinc-500 flex items-center gap-1.5 mb-1">
                      ...
                      Prazo Limite da Rodada {selectBetGroupRound}
                    </div>
                    <div className="text-sm font-sans text-zinc-300">
                      O bloqueio dos palpites ocorre automaticamente se a rodada fechar ou 5 min antes do início de cada confronto.
                    </div>
                  </div>

                  <div className="flex items-center gap-3">
                    {isRoundLocked ? (
                      <div className="px-4 py-2 bg-rose-950/50 border border-rose-500 text-rose-300 font-mono text-xs font-bold rounded-lg flex items-center gap-1.5 font-sans">
                        <Lock size={14} />
                        RODADA BLOQUEADA
                      </div>
                    ) : (
                      <div className="px-4 py-2 bg-emerald-950/50 border border-emerald-500 text-emerald-300 font-mono text-xs font-extrabold rounded-lg flex items-center gap-1.5 animate-pulse font-sans">
                        <Unlock size={14} />
                        FECHAMENTO EM: {timeLeftString}
                      </div>
                    )}
                  </div>
                </div>
          ...
          <div className="text-zinc-400 font-mono text-sm bg-zinc-950/40 p-3 rounded-lg border border-zinc-850 flex items-center justify-between">
            <span>Selecção Ativa: <span className="text-emerald-400 uppercase font-black">{selectedBetGroup}</span></span>
            {(() => {
              let selectBetGroupRound = 1;
              if (["Grupo E", "Grupo F", "Grupo G", "Grupo H"].includes(selectedBetGroup)) {
                selectBetGroupRound = 2;
              ...
              const isGroupOpen = roundStatuses[selectBetGroupRound] === "open";
              return (
                <span className={`text-[10px] border rounded bg-zinc-950 px-2 py-0.5 ${
                  isGroupOpen ? "text-emerald-400 border-emerald-500/20" : "text-rose-450 border-rose-550/20"
                }`}>
                  {isGroupOpen ? "🔓 ABERTO PARA PALPITE" : "🔒 VISUALIZAÇÃO APENAS"}
                </span>
              );
            })()}
          </div>
          ...
          {(() => {
            const filteredMatches = matches.filter((m) => {
              if (selectedBetGroup === "Mata-Mata") {
                return m.roundNumber === 4;
              }
              return m.group === selectedBetGroup;
            });

            if (filteredMatches.length === 0) {
              return (
                <div className="text-center py-20 text-zinc-500 font-sans border border-dashed border-zinc-800 rounded-3xl">
                  Nenhum jogo nesta chave foi cadastrado ou liberado pelo Administrador da sala ainda.
                </div>
              );
            }
            ...
            return (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {filteredMatches.map((match) => {
                  ...
                  const isLocked = isMatchBetLocked(match);
                        ...
                        <div>
                          {isMatchFinished ? (
                            ...
                            <div className="flex items-center gap-2">
                              {/* Status indicators */}
                              {isLocked ? (
                                <span className="text-rose-400 flex items-center gap-1 bg-rose-950/20 px-2 py-0.5 rounded border border-rose-900/30">
                                  <Lock size={12} /> Palpite Travado
                                </span>
                              ) : (
                                <div className="flex items-center gap-2">
                                  {userSavedBet && (
