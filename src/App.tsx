...
export default function App() {
  ...
  const [selectedFixturesGroup, setSelectedFixturesGroup] = useState<string>("Grupo A");
  const [selectedGroupStatic, setSelectedGroupStatic] = useState<string>("Grupo A");

  const handleSelectFixturesGroup = (groupName: string) => {
    sounds.playClick();
    setSelectedFixturesGroup(groupName);
                ...
                {activeTabHomepage === "fixtures" && (
                  <div>
                    <div className="border-b border-zinc-850 pb-3 mb-4">
                      <h2 className="text-lg font-bold text-white flex items-center gap-2 font-display mb-3">
                        📅 JOGOS POR GRUPO / FASE
                      </h2>
                      
                      {/* Group selection pills identical to Copa Groups */}
                      <div className="flex gap-1.5 overflow-x-auto pb-2 scrollbar-thin scrollbar-thumb-zinc-800 scrollbar-track-transparent select-none">
                        {["Grupo A", "Grupo B", "Grupo C", "Grupo D", "Grupo E", "Grupo F", "Grupo G", "Grupo H", "Grupo I", "Grupo J", "Grupo K", "Grupo L", "Mata-Mata"].map((groupName) => {
                          const isSelected = selectedFixturesGroup === groupName;
                          const label = groupName === "Mata-Mata" ? "Mata-Mata" : groupName.replace("Grupo ", "Grupo ");
                          return (
                            <button
                              key={groupName}
                              onClick={() => handleSelectFixturesGroup(groupName)}
                              className={`px-3 py-1.5 rounded-lg text-[11px] font-sans font-extrabold cursor-pointer transition-all flex-shrink-0 ${
                                isSelected
                    ...
                    <div className="text-xs font-mono text-emerald-400 bg-zinc-950 border border-zinc-850/60 p-2.5 rounded-xl mb-4 font-black uppercase text-center tracking-wider">
                      {selectedFixturesGroup === "Mata-Mata" ? "Fase Eliminatória (Mata-Mata)" : `Fase de Grupos • ${selectedFixturesGroup}`}
                    </div>

                    {(() => {
                      const displayedMatches = matches.filter((m) => {
                        if (selectedFixturesGroup === "Mata-Mata") {
                          return m.roundNumber === 4;
                        }
                        return m.group === selectedFixturesGroup;
                      });

                      if (displayedMatches.length === 0) {
                        return (
                          <div className="text-center py-12 text-zinc-500 font-mono text-xs border border-dashed border-zinc-800 rounded-2xl">
                            Nenhum jogo cadastrado para {selectedFixturesGroup} ainda.
                          </div>
                        );
                      }
